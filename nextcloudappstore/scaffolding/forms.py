...
class AppScaffoldingForm(Form):
    ...
    issue_tracker = URLField(
        label=_("Issue tracker URL"), required=True, help_text=_("Bug reports and feature requests")
    )
    categories = MultipleChoiceField(
        required=True,
        label=_("Categories"),
        choices=lazy(get_categories, list),
        help_text=_("Hold down Ctrl and click to select multiple entries"),
    )
    summary = CharField(
        max_length=256,
        label=_("Summary"),
        help_text=_("Short description of your app that will be rendered as short teaser"),
    )
    description = CharField(
        widget=Textarea,
        label=_("Description"),
        help_text=_("Full description of what your app does. Can contain Markdown."),
    )
    opt_in = BooleanField(
        label=_(
...
class IntegrationScaffoldingForm(Form):
    name = CharField(max_length=80, label=_("Integration name"), widget=TextInput())
    author_homepage = URLField(label=_("Author's homepage"), required=False)
    issue_tracker = URLField(
        label=_("Issue tracker URL"), required=False, help_text=_("Bug reports and feature requests")
    )
    categories = MultipleChoiceField(
        widget=HiddenInput(),
        disabled=True,
    ...
    screenshot = URLField(
        max_length=256, label=_("Screenshot URL"), required=False, help_text=_("URL for integration screenshot")
    )
    screenshot_thumbnail = URLField(
        max_length=256,
        label=_("Screenshot thumbnail URL"),
        required=False,
        help_text=_(
            "URL for integration "
            "screenshot in "
    ...
    description = CharField(
        widget=Textarea,
        label=_("Description"),
        help_text=_("Full description of what your integration does. Can contain Markdown."),
    )
    ...
    def save(self, user, app_id, action):
        ...
        try:
            ...
            app = App.objects.create(id=app_id, owner=user, certificate=uuid.uuid1().urn)
            ...
            if user.is_superuser:
                ...
                send_mail(
                    "New integration submitted",
                    "Please review the integration to make sure it fits the guidelines.",
